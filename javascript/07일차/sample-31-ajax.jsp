...
$(function(){
	$(".list-group-item").click(function(){
		...
		// callback: 서버로부터 성공적인 응담이 왔을 때 jQuery가 실행하는 함수 
		//				callback함수의 매개변수에는 서버로부터 받은 데이터가 전달된다. 
		//				응답데이터예시
		//					data = "이것이 자바다, 자바의 정석, 혼자공부하는 자바"
		$.get("ajax1", {genre:text}, function(data){
			// items는 배열이다.
			// items = ["이것이 자바다, 자바의 정석, 혼자공부하는 자바"]
			var items = data.split(",")		// 서버로부터 받은 데이터 분석
			// $.each(배열, 콜백함수)는 지정된 배열에 저장된 요소의 개수만큼 콜백함수를 실행한다.
			...
			$.each(items, function(index,item){		// 새로운 html 컨텐츠 생성
				...
				$("table tbody").append(row);		// 생성된 html 화면에 반영
