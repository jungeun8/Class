...
<div class="container">
	<div class="row mb-3">
		<div class="col-12">
			<h3 class="border p-2 bg-light">나의일정</h3>
		</div>
	</div>
	<div class="row mb-3">
		<div class="col-12">
									...
									<td>
										<c:choose>
											<c:when test="${todo.status eq '등록' }">
												<span class="bage bage-primary p-1">${todo.status }</span>
											</c:when>
											<c:when test="${todo.status eq '완료' }">
												<span class="bage bage-success p-1">${todo.status }</span>
											</c:when>
											<c:when test="${todo.status eq '삭제' }">
											<span class="bage bage-secondary p-1">${todo.status }</span>
											</c:when>
										</c:choose>
									</td>
									<td><fmt:formatDate value="${todo.createdDate }"/></td>
									<td><a href="/todo/del?no=${todo.no }" class="btn btn-outline-danger">삭제</a></td>
								</tr>
							</c:forEach>
						</c:otherwise>
					</c:choose>
