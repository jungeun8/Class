...
// 사용자 정보가 존재하지 않으면 로그인폼을 재요청하게 한다.
if(sessionSavedUser == null){
	String encodedText = URLEncoder.encode("리뷰등록하기", "utf-8");
	response.sendRedirect("../user/loginform.jsp?fail=deny&fail=deny&job=" + encodedText);
	return;
}
...
int no = review.getNo();
reviewDao.getReviewsByProductNo(no);


// 상품 상세 페이지를 재요청하는 url을 응답으로보낸다.
