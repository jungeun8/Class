-- 다중행 함수 
-- employees 테이블에 저장된 모든 사원들의 수 조회하기 
select count(*)
from employees;
...
-- 부서별로 사원들을 그룹화한 다음, 부서별 급여 총합, 급여 평균을 조회하기 
select department_id, sum(salary), round(avg(salary))
from employees
group by department_id;

 -- 부서별로 사원들을 그룹화한 다음, 부서별 급여 총합, 급여 평균을 조회하기 
 -- 부서이름, 부서별 급여총합, 부서별 글여평균을 조회함 
...
-- with절을 사용해서 퀴리 실행속도를 개선 및 쿼리의 가독성을 높이기 
-- 가상의 테이블이 만들어짐 
